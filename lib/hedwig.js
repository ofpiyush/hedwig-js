...
class Hedwig {
    constructor(settings, err_callback, close_callback, consume) {
        if (consume === undefined) {
            consume = true;
        }
        this._should_consume = !!consume;
        ...
        if (!err_callback) {
            err_callback = (err) => {
                console.error({"package": "hedwig", "error": err});
            };
        }
        if (!close_callback) {
            close_callback = () => {
                console.info({"package": "hedwig", "message": "Hedwig connection closed"})
            }
        }
        ...
        this._channel.assertExchange(this._settings.EXCHANGE, this._settings.EXCHANGE_TYPE, {
            ...
            var send_msgs = () => {
                ...
                if (this._pubQueue.length != 0) {
                    setTimeout(send_msgs.bind(this), 100);
                }
            }
            send_msgs();
